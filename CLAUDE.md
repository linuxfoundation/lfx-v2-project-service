...
> - `project-service-code-reviewer` at `.claude/skills/project-service-code-reviewer/` and `project-service-learnings-reviewer` at `.claude/skills/project-service-learnings-reviewer/` are the repo-owned review brains loaded by the work cycle's review subagents — not skills a developer invokes by hand. Their stable path/discovery aliases are `local-code-review` and `local-learnings-review`; the physical skills' declared names are the canonical names used in launch prompts. For each review, the repo-owned skill blob at the pinned `target_sha` is authoritative; a named Skill-tool load is used only when shown equivalent to that blob, otherwise the work cycle follows the pinned blob directly.
...
3. **Launch all three review subagents in one parallel batch as soon as no batch is active.** Use the generic Agent tool (`subagent_type: general-purpose`) three times in a single message, each with `model: opus` and `run_in_background: true`. Child 1 loads only `lfx-skills:lfx-general-code-review`; child 2 loads only `project-service-code-reviewer`; child 3 loads only `project-service-learnings-reviewer`. `local-code-review` and `local-learnings-review` remain stable path/discovery aliases, but launch prompts use the physical skills' declared names. For child 2 or 3, the authoritative skill body is `git show <target_sha>:.claude/skills/<skill-name>/SKILL.md`; a named Skill-tool load may be followed only when the child shows it is exactly equivalent to that blob, otherwise the child follows the pinned blob. Never use a moving working-tree skill file. Child 1 remains an installed central-plugin dependency because its skill is not part of the target repo; if it is unavailable, return `INCOMPLETE` rather than reading a target-repo file.
4. **Post-commit mode prompt for each child (exact; substitute the assigned skill, its skill-source rule, the absolute repo root, pinned full SHAs, and range label):**
   ...
   Read every target-repo review-substrate file selected by the assigned skill from the pinned target with git show <target_sha>:<path>, including the repo-owned skill body and any selected CLAUDE.md, rule, review-checklist, architecture, contract, or routed docs/reviews/knowledge-base/** file. Preserve the assigned skill's routing rules; do not blanket-read unselected substrate. Never use a moving working-tree copy as review-substrate evidence. The installed central general-review skill is not part of the target repo and remains the central dependency.
   For changed-file evidence, read added or modified files with git show <target_sha>:<path>; read deleted files with git show <base_sha>:<path>; and for renames read both git show <base_sha>:<old-path> and git show <target_sha>:<new-path>. Never use the moving working-tree copy as evidence.
   ...
   Lead your report with `Reviewed range: target_sha=<target_sha> base_sha=<base_sha>`, using the full 40-character SHAs. On the next line state one exact source: `Review skill: lfx-skills:lfx-general-code-review; source: installed central Skill tool`, `Review skill: <skill-name>; source: Skill tool verified equal to git show <target_sha>:.claude/skills/<skill-name>/SKILL.md`, or `Review skill: <skill-name>; source: git show <target_sha>:.claude/skills/<skill-name>/SKILL.md`. If the report is INCOMPLETE, the INCOMPLETE line must lead, that exact Reviewed range line must follow, and the Review skill line must be third (use `source: unavailable` only when no skill was loaded).

   Review <range label>.
   ```

   The exact `<skill-source-rule>` for child 1 is: `Load lfx-skills:lfx-general-code-review from the installed central Skill tool. It is not a target-repo file. If unavailable, do not review unguided; return an INCOMPLETE report.` For child 2 it is: `Treat git show <target_sha>:.claude/skills/project-service-code-reviewer/SKILL.md as authoritative. Use a project-service-code-reviewer Skill-tool load only if you show it is exactly equivalent to that blob; otherwise follow the pinned blob directly. Never read the working-tree skill file.` For child 3 it is: `Treat git show <target_sha>:.claude/skills/project-service-learnings-reviewer/SKILL.md as authoritative. Use a project-service-learnings-reviewer Skill-tool load only if you show it is exactly equivalent to that blob; otherwise follow the pinned blob directly. Never read the working-tree skill file.` Append `extra: <focus>` on a new line only when there is a priority hint to add. Do NOT pass `branch` here. If this work cycle is launched from the LFX workspace parent, the `target repo:` and `repo root:` lines keep all three children in this repo.
...
3. **Full-branch sweep — if the branch has more than one commit, or always when the final commit's trio was skipped.** Every sweep launch or relaunch independently runs `git fetch origin`, then pins the full cumulative range with `base_sha=$(git merge-base origin/main HEAD)` and `target_sha=$(git rev-parse HEAD)`. Never use the post-commit coverage boundary as a sweep base. Launch the same three children in one parallel batch (`subagent_type: general-purpose`, `model: opus`, `run_in_background: true`, one physical declared skill name each) with this exact prompt, substituting the assigned skill, its step 4 skill-source rule, the absolute repo root, and pinned full SHAs:
