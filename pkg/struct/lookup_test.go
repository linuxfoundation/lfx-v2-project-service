...
func TestFieldByTag(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, found := FieldByTag(tt.obj, tt.tagType, tt.tagValue)

			if found != tt.shouldFind {
				t.Errorf("FieldByTag() found = %v, want %v", found, tt.shouldFind)
			}

			if tt.shouldFind && !reflect.DeepEqual(result, tt.expected) {
				t.Errorf("FieldByTag() result = %v, want %v", result, tt.expected)
			}

			if !tt.shouldFind && result != nil {
				t.Errorf("FieldByTag() result should be nil when not found, got %v", result)
			}
		})
	}
}

func TestFieldByTagWithPointer(t *testing.T) {
	tests := []struct {
		name        string
		obj         any
		tagType     string
...
func TestFieldByTagWithNonStruct(t *testing.T) {
	tests := []struct {
		name        string
		obj         any
		tagType     string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if found {
				t.Errorf("FieldByTag() found = %v, want false for non-struct input", found)
			}

			if result != nil {
				t.Errorf("FieldByTag() result = %v, want nil for non-struct input", result)
			}
		})
	}
}

func TestFieldByTagEdgeCases(t *testing.T) {
	...
	t.Run("ZeroValues", func(t *testing.T) {
		...
		if !found {
			t.Errorf("FieldByTag() should find field even if it has zero value")
		}

		if result != "" {
			t.Errorf("FieldByTag() result = %v, want empty string for zero value", result)
		}
	})
	...
	t.Run("ZeroValueInt", func(t *testing.T) {
		...
		result, found := FieldByTag(obj, "json", "age")
		...
		if result != 0 {
			t.Errorf("FieldByTag() result = %v, want 0 for zero value int", result)
		}
	})
	...
	t.Run("ZeroValueBool", func(t *testing.T) {
		...
		result, found := FieldByTag(obj, "json", "is_active")
		...
		if result != false {
			t.Errorf("FieldByTag() result = %v, want false for zero value bool", result)
		}
	})
}
